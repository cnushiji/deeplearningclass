...
with tab1:
    ...
    with col1:
        st.image('./pages/图片/图片10.jpg', caption='图10')
    """- GAN的优缺点:"""
    """$ \qquad $ 1.优点："""
...
with tab2:
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./pages/图片/图片12.jpg', caption='图12')
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./pages/图片/图片15.jpg', caption='图15')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./pages/图片/图片97.jpg', caption='图97')
    ...
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./pages/图片/图片100.png', caption='图100')

    """$ \qquad $ 梯度惩罚的选取并不是在全网络下，仅仅是在真假分布之间抽样处理，下图为处理过程。"""
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./pages/图片/图片101.png', caption='图101')
    """$ \qquad $ WGAN-GP的最终目标函数： """
    _, col1, _ = st.columns([1, 4, 1])
    with col1:
        st.image('./pages/图片/图片102.png', caption='图102')
